document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        en: {
            ...
            modalDisruptText: "Drawing on Blommaert & Varis's concept of \"enoughness\", this project reveals how multilingual individuals must constantly perform adequacy within shifting orders of indexicality—the social hierarchies that determine whose language \"counts\" in which spaces. By mapping these feelings of \"not enoughness,\" we turn private anxieties into public data, arguing that everyday multilingual practices are not evidence of lack, but resources to be valued.",
            modalHierarchy: "Challenging Language Hierarchies",
            modalHierarchyText: "Hopkyns & Elyas document the English-Arabic ideological hierarchy in Gulf higher education, where English dominates academic assessment while Arabic is relegated to symbolic heritage. This visualization challenges that hierarchy by making visible where and how Arabic (and other languages) function in learning, mentoring, and peer support—not just in \"cultural\" corners.",
            ...
            student: "Student",
            ta: "Teaching Assistant / Tutor",
            faculty: "Faculty",
            staff: "Staff",
            visitor: "Visitor / Other",
            preferNotToSay: "Prefer not to say",
            mapExplainer: "Each point you see on this map is an anonymous moment when someone at CMU-Q used a language and felt ‘too much,’ ‘not enough,’ or ‘just enough.’"
        },
        ar: {
            title: "إنجليزي كافٍ؟",
            ...
            modalDisruptText: "بالاعتماد على مفهوم \"الكفاية\" لبلومارت وفاريس، يكشف هذا المشروع كيف يجب على الأفراد متعددي اللغات أن يؤدوا باستمرار الكفاية ضمن ترتيبات الفهرسة المتغيرة - التسلسلات الهرمية الاجتماعية التي تحدد أي لغة \"تُحسب\" في أي مساحات. من خلال رسم خريطة لهذه المشاعر بـ\"عدم الكفاية\"، نحول القلق الخاص إلى بيانات عامة، مجادلين بأن الممارسات اللغوية المتعددة اليومية ليست دليلاً على نقص، بل موارد يجب تقديرها.",
            modalHierarchy: "تحدي التسلسلات الهرمية اللغوية",
            modalHierarchyText: "يوثق هوبكينز وإلياس التسلسل الهرمي الأيديولوجي الإنجليزي-العربي في التعليم العالي بالخليج، حيث تهيمن الإنجليزية على التقييم الأكاديمي بينما تُخفض العربية إلى تراث رمزي. يتحدى هذا التصور ذلك التسلسل الهرمي من خلال جعل مرئياً أين وكيف تعمل العربية (واللغات الأخرى) في التعلم والإرشاد ودعم الأقران - وليس فقط في \"الزوايا الثقافية\".",
            ...
            student: "طالب",
            ta: "مساعد تدريس / مدرس خصوصي",
            faculty: "عضو هيئة تدريس",
            staff: "موظف",
            visitor: "زائر / آخر",
            preferNotToSay: "أفضل عدم القول",
            mapExplainer: "كل نقطة تراها على هذه الخريطة هي لحظة مجهولة استخدم فيها شخص ما في جامعة كارنيجي ميلون - قطر لغة وشعر بأنه \"كثير جداً\" أو \"غير كافٍ\" أو \"كافٍ تماماً\"."
        }
    };
    ...
    function updateLanguage() {
        ...
        // Update map explainer
        document.querySelector('.map-instructions-top p').textContent = t.mapExplainer;
        ...
        // Role translations
        document.querySelector('label[for="role"]').textContent = t.yourRole;
        document.querySelector('select#role option[value=""]').textContent = t.selectRole;
        const roleOptions = document.querySelectorAll('select#role option');
        if (roleOptions.length > 1) {
            roleOptions[1].textContent = t.student;
            roleOptions[2].textContent = t.ta;
            roleOptions[3].textContent = t.faculty;
            roleOptions[4].textContent = t.staff;
            roleOptions[5].textContent = t.visitor;
            roleOptions[6].textContent = t.preferNotToSay;
        }
        ...
        modalBody.querySelector('h3:nth-of-type(3)').textContent = t.modalHierarchy;
        modalBody.querySelector('p:nth-of-type(3)').textContent = t.modalHierarchyText;
        modalBody.querySelector('h3:nth-of-type(4)').textContent = t.modalImpact;
        modalBody.querySelector('p:nth-of-type(4)').textContent = t.modalImpactText;
    ...
    function init() {
        ...
        // No need for setInterval - Firebase provides real-time updates automatically
    ...
    const firebaseConfig = {
        apiKey: "AIzaSyB7o6ihJai_xfn2iFl5pV719shxjv8W-6EE",
        authDomain: "enough-english.firebaseapp.com",
        databaseURL: "https://enough-english-default-rtdb.europe-west1.firebasedatabase.app",
        projectId: "enough-english",
        storageBucket: "enough-english.firebasestorage.app",
        messagingSenderId: "490161578656",
        appId: "1:490161578656:web:8ad63a985b71613db55fc5"
    };
    ...
    function initializeFirebase() {
        try {
            ...
            if (firebaseConfig.apiKey === "YOUR_API_KEY_HERE") {
                console.warn("⚠️ Firebase not configured yet. Using local dummy data.");
                console.warn("📝 To enable real-time sharing:");
                console.warn("1. Go to https://console.firebase.google.com");
                console.warn("2. Create a new project");
                console.warn("3. Enable Realtime Database");
                console.warn("4. Copy your config values into script.js");
                loadDummyData();
                return false;
            }
    ...
    function saveEntry(entry) {
        if (entriesRef) {
            ...
            entriesRef.push(entry)
                .then(() => {
                    console.log('✅ Entry saved to Firebase:', entry);
                })
                .catch((error) => {
    ...
    function loadData() {
        if (entriesRef) {
            // Load from Firebase and listen for real-time updates
            entriesRef.on('value', (snapshot) => {
                ...
                if (data) {
                    ...
                    console.log(`📊 Loaded ${state.entries.length} entries from Firebase`);
                } else {
                    state.entries = [];
                    console.log('No entries in Firebase yet');
                }
    ...
    function loadDummyData() {
        ...
        if (savedData) {
            try {
                ...
                console.log('Loaded data from local storage:', state.entries.length, 'entries');
            } catch (e) {
    ...
    function updateEntriesList() {
        ...
        recent.forEach(entry => {
            ...
            li.innerHTML = `
                ...
                <div class="entry-meta">Floor: ${entry.floor || 'Ground'}${entry.role ? ` • ${entry.role}` : ''}</div>
